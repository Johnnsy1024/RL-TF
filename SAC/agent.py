...
class SAC:
    def __init__(self, env: gym.Env, state_dim: int, action_dim: int, hidden_dim: int, actor_lr: float, critic_lr: float, alpha_lr: float, gamma: float, tau: float, alpha_init: float, target_entropy: float, buffer_size: int):
        self.actor = build_actor(state_dim, hidden_dim, action_dim, env.action_space.high[0])
    ...
    def update(self, transition_dict: dict):
        ...
        with tf.GradientTape() as critic_1_tape, tf.GradientTape() as critic_2_tape, tf.GradientTape() as actor_tape, tf.GradientTape() as alpha_tape:
            ...
            critic_1_loss = tf.reduce_mean(tf.square(self.critic_1([states, actions]) - td_target))
            critic_2_loss = tf.reduce_mean(tf.square(self.critic_2([states, actions]) - td_target))
            
            new_actions, log_probs = self.actor(states)
            entropy = -log_probs
            q1_value = self.critic_1([states, new_actions])
            q2_value = self.critic_2([states, new_actions])
            actor_loss = tf.reduce_mean(tf.exp(self.log_alpha) * entropy - tf.minimum(q1_value, q2_value))

            alpha_loss = tf.reduce_mean(tf.exp(self.log_alpha) * (entropy - self.target_entropy))
        
        critic_1_grads = critic_1_tape.gradient(critic_1_loss, self.critic_1.trainable_variables)
        critic_2_grads = critic_2_tape.gradient(critic_2_loss, self.critic_2.trainable_variables)
