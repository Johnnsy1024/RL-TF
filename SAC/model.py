...
def build_actor(state_dim: int, hidden_dim: int, action_dim: int, action_bound: float, actor_type: str = "continuous"):
    if actor_type == "continuous":
        ...
        def sample_action(args):
            ...
            log_prob -= tf.reduce_sum(tf.math.log(1 - tf.square(tf.tanh(normal_sample)) + 1e-7), axis=-1, keepdims=True)    
