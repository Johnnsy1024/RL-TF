        ...
        with tqdm(total=int(n_episodes/10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(n_episodes/10)):
                ...
                while not done:
                    ...
                    value = agent.critic_model(tf.reshape(tf.convert_to_tensor(state, dtype=tf.float32), [-1, state_dim]))
                    values.append(value.numpy())
                    state = next_state
                    episode_rewards.append(reward)
                returns = compute_return(rewards, gamma)
                advantages = returns - np.array(values).flatten()
                train_step(states, actions, returns, advantages, agent.actor_model, agent.critic_model, actor_optimizer, critic_optimizer)
                if (i_episode + 1) % 10 == 0:
                    pbar.set_postfix({'episode': '%d' % (n_episodes/10 * i + i_episode + 1), 'return': '%.3f' % np.sum(episode_rewards)})
                pbar.update(1)
