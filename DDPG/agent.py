...
class DDPG:
    def __init__(
        ...
        self.actor_inputs = tf.keras.layers.Input(
            shape=(state_dim,), name="actor_input: state"
        )
        self.critic_inputs_state = tf.keras.layers.Input(
            shape=(state_dim,), name="critic_input: state"
        )
        self.critic_inputs_action = tf.keras.layers.Input(
            shape=(action_dim,), name="critic_input: action"
        )
        self.actor = build_actor_model(
            action_dim, hidden_dim, self.action_bound, self.actor_inputs
        )
        self.actor_target = build_actor_model(
            action_dim, hidden_dim, self.action_bound, self.actor_inputs
        )
        self.critic = build_critic_model(
            hidden_dim, self.critic_inputs_state, self.critic_inputs_action
        )
        self.critic_target = build_critic_model(
            hidden_dim, self.critic_inputs_state, self.critic_inputs_action
        )
    ...
    def take_action(self, state):
        self.sample_count += 1
        self.simga = self.sigma_end + (self.sigma - self.sigma_end) * np.exp(
            -1.0 * self.sample_count / 1000
        )
        state = tf.reshape(state, [1, self.state_dim])
        action = self.actor(state).numpy()[0]
        action_final = action + self.sigma * np.random.randn(self.action_dim)
        action_final = np.clip(
            action_final, self.env.action_space.low, self.env.action_space.high
        )
        return action_final

    def soft_update(self, net: tf.keras.Model, target_net: tf.keras.Model):
        for var, target_var in zip(
            net.trainable_variables, target_net.trainable_variables
        ):
    ...
    def update(self, trainsition_dict: dict):
        ...
        with tf.GradientTape() as actor_tape, tf.GradientTape() as critic_tape:
            critic_loss = tf.reduce_mean(
                tf.square(q_targets - self.critic([states, actions]))
            )
