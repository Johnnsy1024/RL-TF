...
class PPO:
    ...
    def __init__(
        ...
        actor_input = tf.keras.Input(
            shape=(self.state_dim,), name="actor_input", dtype=tf.float32
        )
        critic_input = tf.keras.Input(
            shape=(self.state_dim,), name="critic_input", dtype=tf.float32
        )
    ...
    def update(self, transition_dict):
        ...
        old_log_probs = tf.math.log(
            tf.gather(self.actor(states), actions, axis=1, batch_dims=1) + 1e-8
        ).numpy()
